    ...
    async execute(client) {
        ...
        setInterval(async () => {
            ...
            for (let i = 0; i < pingsallowed.length; i++) {
                ...
                if (!member.roles.cache.get(process.env.MAINSHR)) {
            ...
            const msg = await client.channels.cache.get(process.env.SHRPings).messages.fetch(process.env.SHRPingsMessage)
